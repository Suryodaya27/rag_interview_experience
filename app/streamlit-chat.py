...
# 🖼️ Page setup
st.set_page_config(page_title="🧠 Leetcode RAG Chatbot", layout="wide")
st.markdown("<h1 style='text-align: center;'>🧠 Leetcode Interview RAG Assistant</h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if companies:
        for c in companies:
            st.markdown(f"- ✅ **{c[0]}** _(added {c[1].strftime('%Y-%m-%d')})_")
    else:
...
if user_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("🤖 Thinking..."):
            start = time.time()
            response, debug = chat_utils.get_contextual_answer(user_input, debug=True)
            duration = time.time() - start

        st.markdown(response)
        st.caption(f"🕒 Responded in {duration:.2f} sec")
